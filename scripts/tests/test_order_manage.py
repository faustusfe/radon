...
class TestFindTrade:
    ...
    def test_finds_by_perm_id_when_provided(self):
        ...
        result = find_trade(mock_ib, order_id=0, perm_id=222222)
    ...
    def test_perm_id_takes_precedence_over_order_id(self):
        ...
        result = find_trade(mock_ib, order_id=10, perm_id=222222)
    ...
    def test_falls_back_to_order_id_when_perm_id_zero(self):
        ...
        mock_ib.openTrades.return_value = [trade1]
        
        # permId=0, should fall back to orderId
        result = find_trade(mock_ib, order_id=10, perm_id=0)
    ...
    def test_returns_none_when_not_found(self):
        ...
        # Non-existent permId
        result = find_trade(mock_ib, order_id=0, perm_id=999999)
...
class TestCancelOrder:
    ...
    def test_cancels_order_by_perm_id(self):
        ...
        result = find_trade(mock_ib, order_id=0, perm_id=326559220)
        
        assert result is not None
        assert result.order.permId == 326559220
        
        # In real code, ib.cancelOrder(trade.order) would be called
...
class TestDuplicateOrderIds:
    ...
    def test_handles_multiple_orders_with_same_order_id(self):
        ...
        mock_ib.openTrades.return_value = [aaoi_order, bkd_order]
        
        # Should find AAOI by its permId
        result1 = find_trade(mock_ib, order_id=0, perm_id=1259686775)
        assert result1 is not None
        assert result1.contract.symbol == "AAOI"
        
        # Should find BKD by its permId
        result2 = find_trade(mock_ib, order_id=0, perm_id=1446612730)
    ...
    def test_handles_multiple_orders_with_same_order_id_10(self):
        ...
        mock_ib.openTrades.return_value = [tsll_order, alab_order]
        
        # Should find TSLL by its permId
        result1 = find_trade(mock_ib, order_id=10, perm_id=326482405)
        assert result1 is not None
        assert result1.contract.symbol == "TSLL"
        
        # Should find ALAB by its permId
        result2 = find_trade(mock_ib, order_id=10, perm_id=326482280)
        assert result2 is not None
